...
**Classe** : représentation abstraite d'un **objet**. On peut aussi dire que c'est un "modèle" pour créer des objets.

Une classe définit un ensemble de **propriétés** (= variables ou constantes) et un ensemble de **méthodes** (fonctions) pour manipuler ces propriétés.
...
La classe elle-même est un modèle à partir duquel on crée des **objets concrets**.

**Objet** : la concrétisation d'une classe.

On crée les objets à partir d'une classe.

**Exemple :** À partir de la classe CompteBancaire, on crée des objets du type CompteBancaire : **$compte1, $compte2...**. Chaque objet contient ses propres valeurs pour chaque propriété de la classe :
...
**Les attributs de la classe doivent être manipulés uniquement par les méthodes de la classe (principe d'encapsulation)**.

Puisque la classe est un modèle, **on peut créer autant d'objets qu'on veut à partir de ce modèle** et ils seront indépendants (plusieurs comptes bancaires : compte1, compte2, etc.). Chaque objet aura des valeurs différentes pour ses attributs (l'attribut **numéro** de **compte1** est 666-666666-666 et le **numéro** de **compte2** est 777-777777-777).

Les classes peuvent représenter des entités concrètes comme des voitures, des maisons, des employés... mais aussi des entités plus abstraites comme un fichier d'image, une connexion à une base de données, une position sur une carte, une figure géométrique, un bouton d'interface graphique, un mouvement dans un jeu... Les possibilités sont illimitées.
...
* Considérez les propriétés et les méthodes qui pourraient constituer les classes qu'on vient de mentionner

*  Quelles méthodes rajouteriez-vous dans la classe CompteBancaire pour la rendre plus complète?
...
Une classe est constituée de **propriétés** (attributs) et de **méthodes**. Pour la déclarer, on utilise le mot-clé `class` suivi du nom de la classe (**exemple :** Personne) et des accolades `{}`.
...
```
    ...
    Note : ici on spécifiera les types des propriétés


Le code précédent devrait se trouver, par convention, dans un fichier qui porte le même nom que la classe suivi de `.class.php`.

**Exemple :** `Personne.class.php`

On déclare toutes les propriétés et les méthodes entre les accolades.
Dans ce premier exemple, nous avons uniquement créé deux propriétés `$nom` et `$prenom`, sans aucune méthode. Pour créer une propriété de la classe, **on doit lui donner un nom précédé du signe `$`**. La syntaxe est la même que pour les variables en PHP. On ajoute le modificateur **public** dont nous parlerons plus tard.
...
Une fois la classe déclarée, on peut l'**instancier**. Instancier une classe signifie créer des objets de cette classe.
Pour ce faire, on utilise l'opérateur `new` suivi du nom de la classe. Comme pour tous les autres types de données, on doit stocker l'objet qu'on crée dans une variable (par exemple : `$person1`, `$person2`...)


**Note :** Créez un fichier (par exemple : `accueil.php`) contenant le code suivant :
...
```php
    ...
    // Création d'un 2ème objet Personne. 
...
**var_dump** nous indique que `$person1` est un **objet** de la classe **Personne**, le premier (**#1**) que nous avons créé de cette classe. Ses propriétés n'ont pas encore de valeur définie (**NULL**). Pour `$person2`, c'est la même chose, sauf qu'il s'agit du deuxième objet créé de la classe Personne. Nous allons maintenant voir comment modifier ces propriétés.
...
Nous avons inclus le mot-clé `public` devant les propriétés, ce qui nous permet d'y accéder (en lecture et en écriture) depuis le code en dehors de la classe.

Pour accéder aux éléments de la classe **depuis l'extérieur** de la classe (qu'il s'agisse d'une propriété ou d'une méthode), on utilise le nom de l'objet suivi de l'opérateur `->` (c'est l'équivalent du point `.` dans d'autres langages orientés objet).
...
**principe d'encapsulation**. Pour simplifier ce concept : **les propriétés de la classe devraient être accédées (lues ou modifiées) uniquement par les méthodes de sa propre classe, pas par un code principal ni par le code d'une autre classe**. Ce principe est **fondamental** dans la POO.
...
Si on ne peut pas accéder directement aux propriétés en dehors de la classe... comment est-ce qu'on peut lire et modifier leurs valeurs ? **On doit créer des méthodes ("get" et "set") dans la classe pour lire/écrire les propriétés**.


## 3.2. Création des méthodes de la classe

Pour le moment on a une classe contenant uniquement des propriétés, mais on va avoir besoin d'avoir des méthodes pour les **lire et modifier**. Les méthodes se rajoutent dans la déclaration de la classe (juste après les propriétés ou juste avant).

**Une méthode est** une fonction qui appartient à la classe et qui manipule les propriétés de la classe (lire ou modifier). Les méthodes reçoivent des paramètres et renvoient des résultats (return).
La syntaxe est pareille.

Créons une nouvelle classe **PersonneP** qui contiendra des méthodes pour lire et modifier la propriété « nom » (getter et setter) :
